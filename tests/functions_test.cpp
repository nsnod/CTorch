...
int main(int argc, char **argv) {
    ...
    */
    ...
    // test mse loss
    Tensor<float> logits({2, 3}); // Batch size of 2, number of classes is 3, test made by CHATGPT HEE HEE
    logits.data_->data_ = {
        1.0, 2.0, 3.0, // First sample
        -1.0, 2.0, -3.0  // Second sample
    };

    Tensor<float> targets({2, 3}); // Batch size of 2, number of classes is 3, test made by CHATGPT HEE HEE
    targets.data_->data_ = {
        1.0, 2.0, 3.0, // First sample
        -1.0, 2.0, -3.0  // Second sample
    };
    
    Tensor<float> mse (logits.shape_);
